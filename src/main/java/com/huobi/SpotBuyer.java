...
public class SpotBuyer implements Job {
    ...
    public void startUp() {
        ...
        System.out.println("现货币种 spot currency: (BTC-USD)");
    ...
    public void priceListener() {
        ...
        while (buyIterator.hasNext()) {
            ...
            if (buyOrder.getState().trim().equalsIgnoreCase("filled")) {
                ...
                StrategyCommon.placeSellOrder(spotAccountId, symbol, new BigDecimal(String.valueOf(buyOrder.getPrice())), buyAmount, spot.getPricePrecision(), spot.getAmountPrecision());
                orderCount.getAndIncrement();
                buyIterator.remove();
            } else if (buyOrder.getState().trim().equalsIgnoreCase("canceled")) {
                logger.error("====== 买单已cancel : " + buyOrder.toString() + " ======");
        ...
        while (sellIterator.hasNext()) {
            ...
            if (sellOrder.getState().trim().equalsIgnoreCase("filled")) {
                ...
                logger.error("====== 卖单已cancel : " + sellOrder.toString() + " ======");
                sellIterator.remove();
            }
        ...
        if (orderCount.get() == i && buyOrderMap.size() == 0) {
            if (i < priceList.size()) {
                ...
                StrategyCommon.placeBuyOrder(spotAccountId, symbol, priceList.get(i), new BigDecimal(usdtAmount), spot.getPricePrecision(), spot.getAmountPrecision());
            }
